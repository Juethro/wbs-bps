...
function AdminEmail(){
    ...
    const deleteUser = (id) => {
        const confirmed = window.confirm("Apakah Anda yakin ingin menghapus email ini?");

        if (confirmed){
            router.delete('/dashboard/email/destroy/'+id);
            fetchData();
        }
    }
    ...
    const fetchData = async () => {
        try {
          const response = await fetch('/dashboard/email');
